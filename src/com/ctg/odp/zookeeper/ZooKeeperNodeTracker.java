...
public abstract class ZooKeeperNodeTracker extends ZooKeeperListener {
    ...
    public synchronized void start() {
        ...
        try {
            if (ZKUtil.watchAndCheckExists(watcher, node)) {
                byte[] data1 = ZKUtil.getDataAndWatch(watcher, node);
                if (data1 != null) {
                    this.data = data1;
                } else {
     ...
     */
    public boolean checkIfRootNodeAvailable() {
        try {
            if (ZKUtil.checkExists(watcher, watcher.getRootZNode()) == -1) {
                return false;
            }
        } catch (KeeperException e) {
            abortable.abort("Exception while checking if basenode exists.", e);
        }
        return true;
    }
}
